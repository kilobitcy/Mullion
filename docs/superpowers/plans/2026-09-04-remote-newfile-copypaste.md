...
fn spawn_paste_task(
    ...
    // 它跟同一份 `policy` 再算一次「保留两者」的新名字,这只证明了**内部
    // 一致性**:判冲突和算新名字用的是同一份快照,互相不会对不上。
    //
    // **没有证明、也没有处理外部陈旧性(TOCTOU)**:用户在冲突框前犹豫的
    // 这几秒到几分钟里,别的进程/别的客户端可能已经在目标目录创建了一个
    // 恰好撞上这里自动生成的新名字的文件——真出现这种情况,下面
    // `overwrite=false` 的传输路径会把它覆盖掉。这是已知的、故意不处理
    // 的限制:概率极低(要求另一方恰好在这个等待窗口期内创建了与自动生
    // 成后缀完全同名的文件),后果有上限(最多覆盖一个无关文件,不是任
    // 意读写)。`copy_tree.rs` 的 `overwrite=false` 路径不带 `-n`/
    // `--no-clobber`——项目里记录的理由是不同版本 coreutils 对这个选项
    // 的退出码不可靠,判断不了「是不是因为已存在才跳过」,所以目前没有
    // 更便宜的堵法。下一个读到这里的人应该带着「这里有一个已知的、有意
    // 留着的 TOCTOU 窗口」这个认知往下走,不是「已经想清楚、没问题了」。
    runtime.spawn(async move {
