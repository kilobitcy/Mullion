...
| # | 决策 | 来源 |
|---|---|---|
| D1 | 归档字段 = `ProjectRecord.archived_at: Option<String>`（RFC3339，app 注入时钟，`default` + `skip_serializing_if`，**不升 schema**） | Q11-B |
...
3. 点「取消归档」→ 回到「在用」。
4. 在搜索框打字 → **tab 栏整条消失**，归档项目也能被搜出来，行的时间列写「已归档 · …」。
