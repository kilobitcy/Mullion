...
| `modal_bg` | `#3f3f3f` | 弹窗底（F203 新增，`egui::Visuals::window_fill`）。**中性灰，故意与本表的冷调蓝紫不同源**：弹窗要浮在近黑的终端上被一眼找到，`bar_status` 对 `panel_bg` 只抬 7/255，用户实报「淹没在黑背景里」。底色抬亮的连带账是次要文字对比度往下走，弹窗里一律用 `fg_muted` 而不是 `fg_dim` |
