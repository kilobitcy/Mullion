...
#### 2.3.1 落地修正：单常数模型不成立，改成两项

按上面的流程实测了四个点（每点 3 轮中位数，单位字节／Buffer）：
...
**跨度 24 倍。** 任何单常数（无论标成 2371 还是 55920）在另一端必错一个数量级，
而这正是 F192 要根除的病——修完计数再留一个错的单价，等于把低报从「计数错」
搬到「单价错」。线性拟合得：

```
一个整形好的 Buffer ≈ 1770 + 269 × 字形数   字节
```

所以最终落地的是**两项模型**：`buffers × BUFFER_FIXED_BYTES + glyphs × GLYPH_EST_BYTES`。
代价是 `CachedRun` 要多存一个 `u32 glyphs`，且它必须**跟着载荷一起进回收池**
（`Vec<(T, u32)>`）——池里躺的多半是长行退下来的 buffer，按固定价计会低报一
个数量级，正是本条要修的病换个地方复发。

字形数只能在整形完那一刻算（`Buffer::layout_runs`）；等到 gauge 采样时再遍历
上千个 Buffer 就跑到帧路径上去了（T3）。所以是**存**不是**算**。

校准守护只留两个端点（`(1, 10_000)` 与 `(200, 500)`），带宽仍是 `[预测/4, 预测×4]`；
实测 2361 vs 模型 2039、55920 vs 模型 55570，两端都在带内。
...
#### 5.3.1 落地修正：收益是 7.2×，不是一到两个数量级；且它挂在字体上

按两项模型算一行 200 格 ASCII 的实际账：

| | Buffer 数 | 记账 |
|---|---|---|
| 合并前（每格一个 run） | 200 | `200 × 1770 + 200 × 269` = 407.8 KB |
| 合并后（`MAX_MERGED_CELLS = 128` ⇒ 2 个 run） | 2 | `2 × 1770 + 200 × 269` = 57.3 KB |

**7.2×**，不是 100×。省掉的只有那 1770 的固定头，269 的字形边际价是省不掉的
——字形该排还得排。把 `MAX_MERGED_CELLS` 设成 128 而不是无穷，是因为再往上
收益已经贴着 269 的渐近线，而单个 Buffer 越长、`ShapedCache` 的命中粒度越粗
（一格变了整段重排）。

**一个真实的性能悬崖，必须写进人工验收清单**：新判据量的是**当前字体链**下的
advance。开发机上没装 Google Sans Code，回退到比例字体，实测 `cell_w=13.8047`
而 `M=13.8047`、`i=4.4453`、`x=9.4688`、`─=9.6328`、`中=16.0`（想要 27.6）——
**连 ASCII 都不再合并**，退化成每格一个 Buffer，比 F195 之前还差。这不是判据
写错了，判据是对的：那个字体确实排不进格子，强行合并就是错位。但它意味着
**字体族名写错／字体没装 = 静默的内存与整形量暴涨**。

诊断指纹已写进 `advance_fits_its_cells` 的文档：实机若见 `text:` 与
`reshape=miss:` 同时异常高，先查字体族名，不要先怀疑这条判据。
...
**落地值**：`POOL_CAP_FLOOR = 256`，`pool_cap_for(n) = max(2n, 256)`，
`truncate` 落在 `end_frame` 的 `retain` **之后**（放前面截的是上一帧的旧池，
本帧刚回收的一批照样无上限——这个变异已配守护）。
池里的条目彼此可替换（取出来都要重新整形），丢哪一头都一样，所以用 O(1)
的 `truncate` 而不是挑着丢。
