...
> **v0.1.13 修正（v0.1.12 实机验收后）：本节的两段式分组已废弃。**
> 改为**一排平铺 7 个纯图标按钮**（第一个就是本节说的 `1屏`，所以它不再是「对原型的
> 补充」而是正式的第一个按钮），按钮上不放任何文字，每个布局长什么样由图标本身表达、
> 文字说明走 tooltip。按钮组不再占独立的 48px 工具栏，改画在**菜单栏同一行、水平
> 居中**（省下一行多的终端高度），「分屏」菜单同时撤掉。
>
> 中途一度定为「平铺 6 个、去掉 1 屏、回单屏走标题条 `×`」，实机看过之后推翻——
> 纯图标按钮下「1 屏」那格图标本身就是最好的说明，没有省的必要。约束落在 `spec.md` F82。
>
> 图标里那几个小方块的几何**由 `preset_tree` + `compute_rects` 推导**
> (`preset::icon_cells`)，不照抄原型 `ICON_LAYOUTS` 的手写表：原型的三屏图标是
> `1.6fr 1fr`，而实际布局已按用户裁定改成等宽，另立一张表会让图标永久骗人，而纯图标
> 按钮的图标是用户判断「点哪个」的全部依据，骗错的代价是真的关掉一个 pane。
...
| 2 / 左右分 | `H(0.5, L1, L2)` |
| 2 / 上下分 | `V(0.5, L1, L2)` |
| 3 / 左大右上下 | `H(0.667, L1, V(0.5, L2, L3))` — **v0.1.13 改为 `H(0.5, …)`** |
| 3 / 右大左上下 | `H(0.333, V(0.5, L1, L2), L3)` — **v0.1.13 改为 `H(0.5, …)`** |
...
> **v0.1.13：`bar_tool` 又退回零引用。** 独立工具栏取消（见 §3 的修正段），按钮组
> 的底色是 `sunken_bg`，那一栏的底色没人用了。token 保留并就地注明原因。
