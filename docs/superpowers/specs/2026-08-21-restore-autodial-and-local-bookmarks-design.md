...
- **会话编辑器要显式保住这份列表**:`session_manager/buffer.rs` 的 `to_draft` 是
  **整份重建** `SftpPrefs`,而本地书签不是表单里的字段 —— 不像
  `preserved_automation` 那样带着走的话,用户在会话编辑器里点一次「保存」就把
  本地书签全清了,**而且静默无提示**。同款坑已经踩过一次(自动化配置),照那条做。
