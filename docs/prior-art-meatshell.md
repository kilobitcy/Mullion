...
6. **我们的块是 64 KiB，不是笔记假设的 32 KiB。** `crates/mullion-app/src/profile.rs:19`
   的 `XFER_CHUNK = 64 * 1024`。它**大于** russh 默认的 `maximum_packet_size`
   （32768），也就是说现在每次 `read_chunk` 本来就可能吃到 short read。
   沿用 64 KiB 还是对齐到 32 KiB，要连着下一条一起定。
