...
| 位次 | 条目 | 可验收判据 | 结论 |
|---|---|---|---|
| 1 | **SFTP 下载流水线** | `ReadTiming.reads` 不变、`read_us` 应降到约 1/N | **做**（已登记 F264 / N10） |
| 2 | **遮挡 / 最小化时冻结渲染** | `profile` 的 `frame` / `present` 计数 | **值得做**，未登记编号 |
| 3 | **回滚态降频** | 同上，滚动阅读时的 `frame` 计数 | **值得做**，未登记编号 |
| 4 | **dev 构建提速**（依赖 opt-level=1 + 本机 target 换 lld/mold） | `cargo test --workspace` 墙钟 | **值得做**，零风险 |
| 5 | **`audit.toml` 式的 RUSTSEC 不可达记录** | 无（工程卫生） | **值得做**，零风险 |
...
**验收判据**（写进 F264 / N10）：同一个文件、同一条链路，下载前后对比
`ReadTiming`——`reads` **不应变化**（总字节数没变，块大小没变），`read_us`
...
其余条目（遮挡冻结、回滚降频、dev 构建、`audit.toml`）**有意不给编号**——
