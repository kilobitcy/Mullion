...
mod tests {
    ...
    /// **两条断言缺一不可,它们挡的不是同一件事:**
    ///
    /// - 上界那条挡「常量被调大」。下面那条 `calls == MAX_PUT_ATTEMPTS`
    ///   两边同源,**对常量的值完全不敏感** —— 把它改成 100 照样绿,
    ///   而真机上一台总是回 409 的服务端会在高延迟链路上连打 100 个往返,
    ///   那个 `spawn_blocking` 线程一堵几分钟、`cloud_in_flight` 全程挂着,
    ///   表现成「备份一直在转、点不动」。
    /// - `calls == MAX_PUT_ATTEMPTS` 那条挡「循环压根没用这个常量」
    ///   (写成 `for _ in 0..8` 或 `loop {}`)。只判区间的话这条逃得掉。
    #[test]
    fn retries_are_bounded_so_a_always_409_server_cannot_spin_forever() {
        // 4 是拍的:撞号意味着**另一台机器**刚推过,再多试也只是跟它抢。
        // 上调之前先想清楚高延迟链路上多打几个往返要堵多久。
        assert!(
            (2..=8).contains(&MAX_PUT_ATTEMPTS),
            "重试上限跑到了 {MAX_PUT_ATTEMPTS} —— 低于 2 等于没有重试(撞一次就失败),\
