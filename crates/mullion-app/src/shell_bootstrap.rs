...
/// 报错(fish 3.x 起本来就默认发 OSC 7,不做兼容,用户可以关掉开关);dash /
/// ash(Alpine、BusyBox 容器的默认 `sh`)同理,而且更彻底 —— `precmd_functions+=(…)`
/// 在**解析**阶段就报错,整条注入行(含末尾 `clear`)一句都不执行,屏幕上只留
/// 一行 `Syntax error`,同样是无害的视觉噪音;tmux
