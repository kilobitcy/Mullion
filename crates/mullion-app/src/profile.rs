...
///
/// F264:64 KiB → 256 KiB。**这是一次实验,不是凭感觉调参**。实测 75 MiB
/// 下载(`f20cf67` 补的那条埋点)是 `reads=1201` —— 正好 1200+1,每次都读满,
/// 512 KB/s,每次往返 127.8 ms,**99.87% 的时间在等往返**。块提到 4 倍之后
/// 那条日志的判读:
///   - 接近 2 MB/s → 往返次数主导,F264 的流水线(多块在途)值得做
///   - 仍卡在 0.9 MB/s 左右 → 链路带宽到顶,F264 该撤销
/// 代价:在途缓冲每条传输也跟着翻 4 倍(记账引用的就是这个常量,自己跟走)。
pub const XFER_CHUNK: u64 = 256 * 1024;
