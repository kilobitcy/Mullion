...
impl ImeState {
    ...
    /// 把光标挪到正在重画的那一段,画完再挪回输入行。跟着走的话,内联拼音和
    /// 系统候选框每次远端重绘都会瞬移一次再弹回来:`/compact` 这类持续刷进度的
    /// 场景下就是肉眼可见的连续闪烁(候选框还额外滞后一帧,一次瞬移闪两下)。
    ///
    /// F211 已经让 tmux 真的把 DEC 2026 同步块转发过来(见
    /// `mullion_store::automation::sync_feature_command`),T2 的攒帧因此第一次
    /// 在主场景里生效,大部分重绘中间帧不会再被画出来。**但这一条仍然必须留着**:
    /// 攒帧只挡住了「远端在同步块里重绘」的那一类,内层程序不发同步块、或者
    /// 攒帧超时(`SYNC_TIMEOUT_MS`)兜底放行时,中间态照样会上屏。
    ///
