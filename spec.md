...
| ID | 指标 | 阈值 | 测法 |
|---|---|---|---|
| N1 | 空闲 CPU 占用 | < 1%（单核） | 连接 4 个 pane 静置 60s，采样。**v0.1.79 起可从一行日志直读**（F179）：`profile.cpu` 的 `total=` 是单核口径两位小数，与 1% 比大小即可。v0.1.78 及更早的日志里它是归一后的整数百分点，`total=0%` 不是达标证据；任何版本都不许拿「窗口没写盘」当证据——空闲门比这条指标松五十倍 |
