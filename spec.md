...
| F169 | **内存分块记账**：`手工记账已知大块 + 显式余量`（不上自定义分配器——为一个诊断指标换全局 allocator，代价与风险都不成比例）。`mem_parts` 渲成 `340MB = scroll:128 xfer:24 text:16 其他:172`，记账超出 RSS 时不吐负数而是 `其他:0(记账超出RSS NMB)` | P2 | `mem_parts` 四条含 `accounted == process_mb` 的相等边界（缺这条时 `<=` 改成 `<` 恒绿，自查挖出）。**单位不对称是有意的**：`process_mb` 是 MB、三个分块参数是字节，各自按 `>> 20` 截断，累计使 `其他` 略偏大（≤3MB），文档注释写明。**回溯口径**：`scrollback_bytes` 按 `grid().history_size()`（**实际已用**行数）计费，不是 `history_lines()`（配置上限）——按上限算的话空 pane 也会报几十 MB。**既有债一并登记**：`diag.rs` 里 `mem_process_mb` 是 `u64` 且采不到时写 0（F155 遗留，`1b04389`），本切片没改数据层类型，改在渲染层兜底成 `profile.mem n/a(RSS 采不到)`；哪天动那块结构体时应改成 `Option<u64>` |
...
| F178 | **白跑帧收口**：掐掉「真实窗口事件 → egui 恒返回 `repaint_delay = ZERO` → 调度路径再转一圈」这个自激环。**F171 的首要嫌疑（幽灵 `CursorMoved`）已被实机数据证伪**：`curdup=0`，286/286 个窗口无一例外，3866 次 cursor 事件的坐标**全都在真的变化**，所以「按坐标去重」这条路作废。真凶是**放大**而非伪造——实机 `scene=ui-only` 的窗口：`frame=39 present=0 wake=90 rr=sched:55,evt:37 wev=cursor:36`（36 个真实鼠标移动 → 90 次唤醒 → 39 帧 → **一帧都没提交**），最坏 `frame=170 present=27 wake=431 rr=sched:270,evt:165 wev=cursor:153` —— 放大约 **1:2.5**、浪费率 84%、主线程 CPU 4%。**`sched` 恒大于 `evt`，多出来的那一半就是自激**。F159 整帧指纹已把这些帧 100% 挡在 GPU 之外（`present=0`），但 CPU 侧的 tessellate + `text_prepare` + 指纹计算全跑了——**省了 GPU 没省 CPU，收口点必须前移到「决定要不要画」，而不是「画完再判断要不要提交」** | P1 | **不许只掐一处**（F171 已论证、本轮数据复核）：当前 431 次唤醒里只有 170 次过帧闸（39%），**帧闸正在吸收自激**；去掉调度路径后剩下的 165 次事件驱动唤醒全在 60fps 以下、100% 会过闸，帧数反而升。**判在结果上，不判在事件类型上**（ADR-011）：不许按「哪个事件是真的」分类——鼠标移动会改 hover 高亮/光标形状/tooltip，那些是真实视觉变化，掐掉就是交互反馈丢失；唯一可信的判据是**整帧指纹连续命中**（画面确实没变），一帧 miss 立刻复位。**硬信号必须强制复位**：`textures_delta` 非空、`terminal_dirty`、`surface.configure`、`AtlasFull`（同 F159 的五条）。**T7 是这条的雷区**：动的正是 `ControlFlow::WaitUntil` 那块，事件循环三分支须显式复位 control_flow，守护 `frame::tests` 的 `plan` 四条决策必须同步扩。**验收要能自证**：拿 `scene=ui-only` 且 `present=0` 的窗口做基线，`frame` 应从 39/57/64 降到接近 0 而 `present` 不变（一帧真内容都不许丢）；`rr=sched:` 与 `evt:` 的比值应跌到 1 以下。**已识别、本条不做的线索**：IME 事件的放大倍数比鼠标更高（`ime:15 → wake=62 → frame=33`，**1:4.1**），且出现在 `key=0x` 的窗口——用户没打字却在收 IME 事件，全局 798 次仅次于 cursor 的 3866 次；它落在 T10/F149 那片区域，单独一轮处理，**不要顺手掺进这条** |
| F179 | **CPU 口径的分辨率**：`profile.cpu` 的 `total=` 按核数归一后取整数百分比，在 16 核机器上**分辨率是 6.25% 单核一档**——而 N1 要求「空闲 CPU < 1%（单核）」= 0.0625% 归一。**它永远显示 0%，达标与超标六倍长得一模一样**：实机 34 分钟 286 个窗口，`total` 的 p50/p95/max 全是 0%。N1 目前**没有观测能力**，不是「已达标」 | P1 | 两个口径都要提精度（`main=` 不归一，但 1% 单核的阈值同样落在整数分辨率的边界上，实机 `ui-only` 场景 p50=0%/p95=2%/max=4% 读不出结论）。**判据：N1 的验收必须能从一行日志直接读出来**，否则这条指标永远无法收口——报一位小数、报千分比、或直接报窗口内的原始 CPU 时间增量（ns），三选一皆可，但不许再出现「0% 同时兼容两种事实」。**连带发现，一并处理**：`IDLE_CPU_PCT = 5` 是**归一后**的阈值，16 核机器上等于 0.8 个核——F164 那条「CPU 超阈值强制打破空闲门」的保险在多核机上事实上从未触发（实机 `main` 最高 12%，`IDLE_MAIN_CPU_PCT = 20` 也没触发过）。因此**「窗口没写盘」不能推出「那一窗口达标 N1」**，空闲门只保证 total<5% 归一；任何拿「未写盘窗口数」当 N1 证据的推理都是错的，注释里要写死这一条 |
| F180 | **脱敏器的 token 收录判据**：`redact.rs` 的 `is_host('-')` 返回 true，于是 `-` 在 `<用户>@-/6`（`profile.pane` 行远端状态未知时的写法）里被 `replace_user_at_host` 当主机名收进 map；一旦进了 map，`replace_known_bare_tokens` 就把**整份日志里所有孤立的 `-`** 都换成同一个假名。实测一份 34 分钟的日志里：184 处 `wev=-`、184 处 `dirty=-`、439 处 pane 行的 `@-/6` 全变成 `host#2`——**凭空多出一台主机**，而 `-` 恰恰是 F157/F171/F173 三处埋点明定的空表占位符（`profile.rs:507`：「空表报 `-` 而不是空串——`dirty=`/`wev=` 后面什么都没有 = 埋点根本没接上」）。脱敏把那个占位符变成了一个假主机名，**正好摧毁它存在的理由** | P2 | 收录候选进 map 前要求**至少含一个 ASCII 字母或数字**且**长度 ≥ 2**：纯标点／单字符 token 不可能是主机名，而它们恰恰是格式串里最常见的占位与分隔符。**假名编号单调增长本身是个信号**：同一份日志里 `profile.pane` 段出现了 `user#265`、`user#266`、`user#267`… 逐窗口递增，说明模式匹配把远端状态串（tmux 会话名／目录名）当成了 `user@host`，每个窗口都在生成新假名——守护里加一条「一份典型日志脱敏后的假名总数有上界」，编号跑飞时当场变红。**不要放宽到「不替换任何短 token」**：单字母主机名是合法的，判据是「含字母数字且长度 ≥2」，不是「长度 ≥3」 |
...
| ID | 指标 | 阈值 | 测法 |
|---|---|---|---|
| N1 | 空闲 CPU 占用 | < 1%（单核） | 连接 4 个 pane 静置 60s，采样。**目前测不了**——`profile.cpu` 的 `total=` 归一后取整、在 16 核机上一档就是 6.25% 单核，这条阈值整个落在一档之内（F179）。别把日志里的 `total=0%` 当达标证据 |
...
| **插队** | F178–F180 | 白跑帧收口 + CPU 口径分辨率 + 脱敏器 token 判据（2026-08-28）。**全部由一份 v0.1.78 的实机日志驱动**（34 分钟 / 286 个窗口）：F171 的幽灵事件假设被证伪、N1 被发现根本没有观测能力、空表占位符 `-` 被脱敏成假主机名。同批把 F174/F175 补登记进本表——它们随 v0.1.76 已交付且有守护测试，spec 里一直缺行 |
