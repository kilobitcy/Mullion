...
| F216 | **内置编辑不再让远端刷屏**：底部「编辑中」列表只列 `EditKind::External`，内置编辑（`Inline`）登记进 `EditSessions` 但不进这个面板 | P2 | 用户实报「点『在 Mullion 里编辑』，后面的 pane 整体抖一次、像刷了屏；关掉编辑窗再抖一次」。**根因不在渲染层，在布局**：那个列表是 `TopBottomPanel::bottom`，按「空则不画」的纪律工作，于是内置编辑一开，列表从空变非空 → 中央区矮三四十点 → `central_px` → `compute_geoms` → `apply_geometry` 发一次 `window_change`（T4）→ 远端 tmux 整屏重排；关窗时列表变空，再来一次。**只滤显示不动 `sessions`**：退出确认（D3-12）与临时文件清理仍要看得见内置那几条，它们照样会走到 `Uploading`/`Conflict`/`Failed`。外部编辑反过来必须留在列表里——它没有窗口，那一行是「我还欠着一次回传」的唯一可见性来源，那次 `window_change` 是它该付的代价 |
| F217 | **编辑器窗口尺寸可控**：正文与底部按钮行改成「`bottom_up` 里先摆底部、剩余矩形整块给正文且 `auto_shrink` 关掉」，删掉猜出来的 `reserve`；补 `min_size` 下界 480×320；开窗只钉位置不再钉尺寸（尺寸记在 egui `Memory` 里）；`resize_grab_radius_corner` 10 → 9 | P2 | 用户实报三条，同一个根因：「载入时高度自己长、有增长动画」「高度调不动」「四角拖不出对角」。**根因是 egui `Resize` 的棘轮**：`desired_size = desired_size.max(last_content_size)`（0.30 的 `containers/resize.rs:258`，只涨不缩），而正文高度过去是 `available_height - reserve` 反推的、`reserve` 又是照按钮行**猜**的常数——猜小的那点差额每帧累积，窗口一路长到 F208 的 85% 天花板（那段「增长动画」与文件载入无关，是帧数）；到顶之后往外拖被夹住、往里拖被下一帧顶回去，于是「调不动」，四角对角拖的竖向分量同样被吃掉，看起来成了「角上只能改宽」。改成「内容高度**恒等于**窗口高度」之后那句 `max()` 永远是 no-op。**下界要显式补**：过去「拖不再小」是正文 `desired_rows(20)` 坠出来的副作用，棘轮一拆地板跟着没了，而 `Resize` 的默认下界是 16×16。**四角另有一份独立的坑**：`hit_test.rs:416` 的 `should_prioritizie_hits_on_back` 在距离打平时「优先选细的命中目标」，判据 `back <= 0.5 * front && back <= 16` 量的是短边——边带 `2×5=10`、角 `2×10=20`，`10 <= 0.5×20` **正好取等**，正角上永远是边带赢、角一次也轮不到；把角半径压到 9（短边 18）打破取等。**不动 `resize_grab_radius_side`**：`TopBottomPanel`/`SidePanel` 的分隔条也读它 |
| F218 | **从终端选区里的路径跳转文件侧栏**：`Ctrl+Shift+B`（叠加在原来的开关栏上）读焦点 pane 的划选文本，整段能解析成路径就跳过去——三态判据是「栏关着 → 开栏并跳；栏开着但不在那个路径 → 跳；栏开着且已经在那个路径 → 关栏」。解析层是纯函数 `files::reveal`（`parse`/`plan`/`arrived`/`consume`），归一化一律委托给 F131 的 `resolve_remote_input`/`resolve_local_input`；栏别按**语法**判（`C:\`、`C:/`、UNC `\\server\share` → 本地栏，其余含相对路径全部 → 远端栏）；相对路径的基准取焦点 pane 的 cwd（与 `files_start_dir` 同一条腿，展开 `~`），取不到才退到面板当前目录。目录/文件之分走一次 SFTP `stat`，指向文件就进**父目录**并把那一项选中 + 滚到可见（不是把列表滤成一条）。跨节点时按 F132 把本标签的 SFTP 换到该 pane 的主机；意图存在标签上、单槽后来居上、在 `accept_sftp_opened` 处按节点身份核对后消费，失败／断线／用户自己翻目录／关标签都撤，**不设超时** | P2 | 用户诉求是「在 Claude Code 的输出里看见一个路径，想立刻在文件栏里打开它」。几处判据是被现有约束逼出来的：**① 键不能用 `Ctrl+B`** —— 那是 tmux 的 prefix，本项目的主场景就是操作远端 tmux 里的 TUI，占了它等于永久废掉用户的复用器；`Ctrl+Shift+B` 本来就是开关栏，叠上去语义连贯。**② 必须整段是路径，不从句子里刨** —— 多行直接拒；成对引号、`:12`/`:12:34` 行列号后缀、首尾标点（中英文各一套）先剥掉再判。刨的代价是误跳：用户划一句话里带个斜杠就把面板拽走，而**跳错和没跳一样贵**。同理加了「必须含分隔符或 `~` 前缀」这道闸，否则划中一个单词 `app.rs` 就会换来一次「找不到」的 toast 噪声。**③ 目录还是文件不能按扩展名猜** —— 远端多的是没有后缀的可执行文件和带点的目录名，猜错就是「进不去还把当前目录弄丢了」；一次 `stat` 换准确，而 `stat` 失败时**只弹 toast、绝不移动 cwd**（把用户已经站定的目录换掉，比什么都不做糟）。注意 SFTP 的 `stat` 是 lstat 语义、不跟随符号链接，指向目录的软链会被当成文件呈现在父目录里——这正是我们要的可见性。**④ 「只显示那一个文件」被否掉** —— 滤成一条之后用户看不见同目录的兄弟文件，而「跳过去」的下一步动作十有八九就是操作旁边那几个；改成「进父目录 + 选中 + 滚到可见」既到达又不丢上下文。**⑤ 意图不设超时**：高延迟代理链路上任何拍脑袋的超时都是 T11 那类「本机全绿、真机才炸」的坑，改成按事件消费 + 按节点身份核对，换节点了就明说「跳转已取消」。开栏那一帧还要**压掉 F123 的「关→开」同步**，否则面板会先被拽到 pane 的 cwd、再被跳转结果覆盖，多一次 readdir 和一次可见的闪。焦点始终留在终端，选区不清；SFTP 节点标签没有终端可读，退化成纯开关 |
| F219 | **远端栏就地新建文件**：右键菜单「新建文件」（**不带省略号**——省略号是弹框的记号，这条不弹框）或 `Ctrl+N`，列表**第一行**长出一个只存在于本地的输入行；回车才发一次带 `EXCLUDE` 的 create，Esc／点别处／焦点被抢一律放弃，远端一个字节都没动。名字过 `files_dialog::validate_name` 与「这个目录里已经有同名」两道闸，非法画红框 + 悬停说原因、**不收走输入框**（敲到一半暂时非法是常态）。建成后刷新并把光标落到新文件上（复用 F218 的 `reveal_pick`）。**远端栏专有**：本地栏收到只记一条 warn 丢掉（设计 D5，本地侧写操作不在本切片范围） | P1 | 判据几乎全是「错了也不报错」那一类：**① `EXCLUDE` 不能省** —— 不带的话在已经有 `config.yaml` 的目录里手滑打了同名，那份配置就被静默截断成 0 字节。**② 幽灵行的索引偏移** —— 行数 `rows.len() + new_row`、行体 `rows[ix - new_row]`，两处共用同一个 `new_row`；错了就是「点第二行选中第一个文件」，而下一步动作十有八九是删除。守护**不能靠「点一下看选中谁」**：绘制与命中拿的是同一个循环变量，`ix` 再怎么错两者永远自洽（重言式），只能从 `Shape::Text` 还原「屏幕自上而下第 k 行画的是谁」再与 `rows()` 逐条比；且样本必须**越过 `show_rows` 的虚拟滚动阈值**（一屏画不完时它压根不迭代到最后一行——2 条样本时是靠越界 panic 蹭的红，30 条以上变异就活了）。**③ 与 `rename_edit` 互斥** —— 两个 `TextEdit` 同时活着互抢焦点，先进编辑态那个永远退不出来（F131 实测）；故 `begin_rename` 清 `new_edit` 那句在函数体第一句，在任何早退**之前**。**④ 列目录结果回来（`accept`）故意不清新建态** —— 它不绑定任何已有行，没有「那一行没了」这回事，跟着 `rename_edit` 一起判会把用户正在打的字吞掉；但换目录／换机器／显式刷新（都过 `begin_load`／`invalidate`）必须清，否则回车拿**新** cwd 拼路径，文件建在用户根本没在看的目录里。**⑤ 必须登记进 `Modal`** —— 枚举、`Modal::ALL`、`modal_open` 独立一臂、判据函数，一处不落，否则 T8：一个键都收不到，Backspace 还会跳去上级目录。**⑥ `reveal_pick` 必须落在 `Refresh` 之后** —— `begin_load` 会 `clear_selection`，F218 同款顺序陷阱；而「发完之后再做 X」的起算点只能是 `SftpOpDone`，不能是调用点（T11） |
| F220 | **远端内复制／剪切／粘贴**：远端栏 `Ctrl+C`／`Ctrl+X`／`Ctrl+V` 与右键三项，剪贴板 **per-tab**（切到别的标签就是空的），剪切后源行画淡。粘贴 = 自身／子孙闸门 → 列一次目标目录做预检查 → 有同名才弹批量框（覆盖／跳过同名／保留两者，**对整批生效**，绝不逐条问）→ `exec cp -a`／`mv` 快路径，被拒回退 SFTP 逐文件递归。同目录复制直接走「保留两者」，同目录剪切是空操作。剪切**落地之后**才清剪贴板，复制不清（连着粘几个目录是常见用法） | P1 | 这条的判据几乎全都长在「异步窗口期里活状态变了」这一个形状上，前后被复核挖出四处：**① 请求参数必须在按下 `Ctrl+V` 那一刻冻结** —— 目标目录 `dst`、剪贴板 `clip`、目标目录已有名字 `existing` 全部随请求走，之后**一律不许从活面板重读**。重读的代价是绕过自身／子孙闸门：用户在往返窗口里切个目录，就能往一个从没检查过的目录里静默覆盖。冲突框把这个窗口从「一次网络往返」拉长到「人在框前思考几秒到几分钟」，所以冻结要一路贯到真正发字节的地方。**② 这条约束靠类型层保证，不靠约定** —— `dispatch_paste` 拆成「`&mut self` 只取 client／conn」+ 自由函数 `spawn_paste_task`（没有 `&self`／`&Tabs` 参数，作用域里连 `self`／`tab` 标识符都不存在，想现读面板是编译错误），参数列表另有白名单守护钉死，加参数必须同步改白名单。**源码切片式的黑名单判据在这条链上被绕过四次**（换绑定名、直接摸 `TabContent` 字段、加一个不撞词的参数、先合规调用再 shadow），每次都编译通过且全绿。**③ `existing` 取预检查时服务器 `list_dir` 的实况，不是面板缓存** —— 拿陈旧列表算「保留两者」的新名字，会生成一个服务器上已经存在的名字并盖掉它。**④ 剪贴板清空要认身份不认时机** —— 剪切 A 发起粘贴、在途时用户又复制了 B，完成事件清的是「此刻的」剪贴板就把 B 误清了；`OpFollow::ClearClip` 携带冻结的那份 `clip`，比对相等才清。同族的还有 **T11**：清空只能挂在 `SftpOpDone` 的成功分支上，发出那一刻就清的话链路一断就是「剪贴板没了、东西还在原地」。**⑤ 目录刷新必须无条件** —— 挂在 `follow` 某个取值上的话，另一条腿（比如纯复制）会静默不刷新、面板停在旧内容且零报错；判据用花括号嵌套深度守，不用「字面串出没出现」。**⑥ 「跳过同名」不用 `cp -n`** —— coreutils 9.2 反转过跳过时的退出码（9.3 又改回），撞上那个版本会被 `ExecOutcome::succeeded` 判成失败；改成客户端按预检查结果过滤。**⑦ 「保留两者」同一批里的新名字要互相避让**，否则两条都叫 `a (副本).txt`、后一条盖掉前一条。**⑧ 自身／子孙闸门必须排在任何请求之前**：远端 `cp` 自己会拦，但 SFTP 回退是我们写的递归，会边列源边往源子孙里写、一直递归到磁盘满；判据是字节前缀 + `/` 边界（`/a/bb` 不是 `/a/b` 的子孙），且**两个方向都要判**——`overwrite` 那条腿先 `rm -rf` 目标，目标是源的祖先时会把源先删掉，把真 `cp` 的一句拒绝变成不可逆销毁。**⑨ 绝不跟随符号链接**（同 F57），回退路上用 `read_link` + `symlink` 原样重建。冲突框的取消 = 整批不动（`cancel_op` 归 `None` 组，与 `Conflict`／`EditConflict` 正相反）。**已知限制**：`existing` 是预检查那一刻的快照，用户在冲突框前犹豫期间别的客户端若恰好创建了与自动生成新名字同名的文件，会被覆盖——概率极低、后果有上限（最多一个无关文件），且 `-n` 因退出码不可靠而用不了，故有意不处理 |
...
| **插队** | F216 | 内置编辑不再让远端刷屏（2026-09-02，v0.1.93）。用户实报「一开编辑窗后面的 pane 就刷一次屏」。根因在布局不在渲染：底部「编辑中」列表是 `TopBottomPanel`，从空变非空就把中央区吃矮 → `central_px` → 一次 `window_change`（T4）→ tmux 整屏重排
| **插队** | F217 | 编辑器窗口尺寸可控（2026-09-02，v0.1.93）。用户实报的三条（自己长高／高度调不动／四角拖不出对角）共一个根因：egui `Resize` 的棘轮 + 猜出来的 `reserve`。另有一份独立的坑在 egui 的命中判定——正角上细边带永远赢过角
| **插队** | F218 | 从终端选区里的路径跳转文件侧栏（2026-09-04，v0.1.94）。`Ctrl+Shift+B` 叠加：整段能解析成路径就跳过去，三态（关着→开并跳／开着不在那→跳／开着已在那→关）。栏别按语法判、相对路径以 pane 的 cwd 为基准、目录还是文件走一次 `stat` 不靠扩展名猜；指向文件是「进父目录 + 选中 + 滚到可见」，不是把列表滤成一条
| **插队** | F219 | 远端栏就地新建文件（2026-09-05，v0.1.95）。`Ctrl+N`／右键，列表第一行长出幽灵输入行，回车才发一次带 `EXCLUDE` 的 create；名字两道闸、非法画红框但不收走输入框；建成后刷新再 `reveal_pick`
| **插队** | F220 | 远端内复制／剪切／粘贴（2026-09-05，v0.1.95）。剪贴板 per-tab，`exec cp -a`／`mv` 快路径 + SFTP 递归回退，同名撞车弹一个对整批生效的批量框。请求参数在按下 `Ctrl+V` 那一刻全部冻结、之后不许重读活面板——这条靠自由函数 `spawn_paste_task` 的签名在类型层保证，因为源码切片式的黑名单判据在这条链上被绕过了四次
